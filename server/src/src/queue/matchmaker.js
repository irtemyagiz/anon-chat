...
function initMatchmaker(io, socket) {
  ...
  socket.on('match:soulmate:start', async () => {
    ...
    if (!isPlus(user)) {
      resetCounterIfNeeded(user, 'dailySoulmateCount', 'lastSoulmateResetAt');
      if (user.dailySoulmateCount >= 5) {
        return socket.emit('match:soulmate:limit', {
          error: 'daily_limit_reached',
          limit: 5,
          used: user.dailySoulmateCount,
        });
      }
    }
    ...
    SOULMATE_POOL.set(user.id, { socket, user, since: Date.now() });
    socket.emit('match:soulmate:waiting');
    console.log(`[match-soulmate] bekliyor: ${user.id}`);
  });
